...
def process_results(client, results, target_name: str, operation: str) -> None:
    ...
    for result in results:
        ...
        if num_failed > 0:
            ...
            logger.error(f"Failed records saved to: {rejected_file}, for {operation} on {target_name}")
        else:
            logger.info(f"Job {job_id} succeeded for {operation} on {target_name}")

def _save_rejected_records(failed_records: str, target_name: str, job_id: str, operation: str) -> str:

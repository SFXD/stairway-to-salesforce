...
from dlt_salesforce_advanced.drivers.salesforce_driver.sfdriver_specs import (
...
class TestGetSalesforceDriver:
    ...
    def setup_method(self):
        ...
        from dlt_salesforce_advanced.drivers.salesforce_driver.sfdriver_cache_manager import clear_cache
        clear_cache()
    ...
    def test_get_driver_different_paths_different_cache(self, mock_secrets, mock_make_driver):
        ...
        mock_secrets.__getitem__.side_effect = [
            {"user_name": "dev@example.com", "password": "pass", "security_token": "token"},
            {"user_name": "prod@example.com", "password": "pass", "security_token": "token"}
        ]
        ...
        @patch('dlt_salesforce_advanced.drivers.salesforce_driver.sfdriver.with_config')  
        @patch('dlt_salesforce_advanced.drivers.salesforce_driver.sfdriver.make_salesforce_driver')
        @patch('dlt.secrets')
        def test_get_driver_with_custom_session(self, mock_secrets, mock_make_driver, mock_with_config):
    ...
    @patch('dlt_salesforce_advanced.drivers.salesforce_driver.sfdriver.with_config', side_effect=lambda f: f)
    @patch('dlt_salesforce_advanced.drivers.salesforce_driver.sfdriver.make_salesforce_driver')
    @patch('dlt.secrets')
    def test_get_driver_with_dict_credentials(self, mock_secrets, mock_make_driver, mock_with_config):
...
class TestGetSalesforceDriverCaching:
    ...
    def test_cache_multiple_environments(self, mock_secrets, mock_make_driver):
        ...
        mock_secrets.__getitem__.side_effect = [
            {"user_name": f"env{i}@example.com", "password": "pass", "security_token": "token"}
            for i in range(4)
        ]
        
        envs = ["salesforce.dev", "salesforce.staging", "salesforce.prod", "salesforce.sandbox"]
...
class TestGetSalesforceDriverIntegration:
    ...
    @patch('dlt_salesforce_advanced.drivers.salesforce_driver.sfdriver_factory.Salesforce')
    @patch('dlt.secrets')

...
def build_resource( sobject: str, fields: list[str], write_disposition: str = "append", primary_key: Optional[str] = None,  replication_key: Optional[str] = None):
    """Factory for DLT resource dynamically wrapping Salesforce client."""

    # Create the incremental cursor if provided, outside the inner function
    ...
    def sf_dynamic_resource(credentials: SalesforceAuth = dlt.secrets.value,session: Optional[Session] = None, incremental_load=incremental_cursor):
        ...
        yield from get_records(sf=client, sobject=sobject, fields=fields, replication_key= replication_key, last_state=last_value )
